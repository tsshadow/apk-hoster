...
from config import APP_NAME, PORT, ALLOWED_IPS, DIST_DIR, logger
...
templates.env.filters["format_size"] = lambda s: humanize.naturalsize(
    s, binary=True
).replace("i", "")
...
def check_permission(
    user: Dict[str, Any], permission: str, apk_name: Optional[str] = None
) -> bool:
    """
...
async def login_get(request: Request):
    ...
    """
    return templates.TemplateResponse(
        request=request, name="login.html", context={"app_name": APP_NAME}
    )
...
async def login_post(
    ...
    return templates.TemplateResponse(
        request=request,
        name="login.html",
        context={"app_name": APP_NAME, "error": "Invalid username or password"},
    )
...
async def edit_user_get(
    request: Request, username: str, admin: bool = Depends(is_admin)
):
    """
...
async def get_version(
    request: Request,
    apk: str = "ultrasonic",
    user: Dict[str, Any] = Depends(require_permission("view")),
):
    """
    ...
    if not check_permission(user, "view", apk):
        raise HTTPException(
            status_code=403, detail=f"Forbidden: No permission for APK {apk}"
        )

    row = db.fetchone(
        "SELECT * FROM apks WHERE apk_name = ? ORDER BY version_code DESC, build_date DESC LIMIT 1",
        (apk,),
    )
    if not row:
        row = db.fetchone(
            "SELECT * FROM apks ORDER BY version_code DESC, build_date DESC LIMIT 1"
        )
    ...
    version_info = {
        ...
        "buildDate": v["build_date"].isoformat()
        if isinstance(v["build_date"], datetime)
        else v["build_date"],
        "filename": v["filename"],
        "size": v["size"],
    ...
    return JSONResponse(
        content=version_info, headers={"Access-Control-Allow-Origin": "*"}
    )
...
async def add_apk(
    ...
    if "admin" not in perms and "upload" not in perms:
        raise HTTPException(
            status_code=403, detail="Forbidden: Missing upload permission"
        )
...
async def serve_file(
    filename: str, user: Dict[str, Any] = Depends(require_permission("download"))
):
    """
    ...
    if row and not check_permission(user, "download", row["apk_name"]):
        raise HTTPException(
            status_code=403, detail="Forbidden: No download permission for this APK"
        )
